...
export const enrollLearner = async (
  ...
  try {
    ...
    const learner = await userService.create({
      ...learnerData,
      user_type: "learner",
    });
...
export const resetPassword = async (
  ...
  try {
    ...
    const { id: user_id } = await userService.verifyPasswordResetLink(
      email_hash,
      hash_string
    );
...
export const addSchoolStudents = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const listSchoolStudents = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const { learners: students, grades } = await userService.listSchoolStudents(
      schoolId,
      queryParams
    );
    sendResponse(res, 200, "Students Fetched", {
      students,
      grades: [...grades],
    });
  } catch (err) {
...
export const downloadSchoolStudents = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
