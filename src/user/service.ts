...
export const userService = {
  async authenticate(
    ...
    if (!foundUser)
      throw new handleError(404, "Email or password is incorrect");

    if (foundUser.deleted)
      throw new handleError(404, "This user has been deleted");

    const match = await bcrypt.compare(password, foundUser.password);
    if (!match) throw new handleError(400, "Email and password doesn't match");

    if (foundUser.status == EUserStatus.Inactive)
      throw new handleError(422, "User account is inactive.");
    ...
    if (foundUser.active_organization)
      payload.organization = foundUser.active_organization;

    let user_type;
    if (foundUser.role != USER_TYPES.admin) {
      user_type = await userModel[foundUser.role]
        .findOne({ user: foundUser._id })
        .select({ user: 0 });
    }
  ...
  async create(
    ...
    user: { school_id: string; role: string } | undefined
  ): Promise<{
    status: string;
    message: string;
    data: IUserView | IUserCreate;
  }> {
    ...
    try {
      ...
      // return newUser;
      return { status: "success", message: "success", data: newUser };
    } catch (err: any) {
  ...
  async createLoginUser(userData: IUserCreate): Promise<ObjectId> {
    ...
    const data: Record<string, any> = getValidModelFields(
      userModel["user"],
      userData
    );
    ...
    if (user_type === USER_TYPES.learner && (parent || school)) {
      data.username = await this.ensureUniqueUsername(
  ...
  async activateAccount(
    ...
    const hash = crypto
      .createHash("md5")
      .update(user.email + process.env.EMAIL_HASH_STRING)
      .digest("hex");
    if (hash_string !== hash) {
  ...
  async verifyPasswordResetLink(email_hash: string, hash_string: string) {
    ...
    if (!user)
      throw new handleError(
        400,
        "Invalid email hash. couldn't verify your email"
      );

    const hash = crypto
      .createHash("md5")
      .update(email + process.env.EMAIL_HASH_STRING)
      .digest("hex");
    if (hash_string !== hash) {
      throw new handleError(400, "Invalid hash. couldn't verify your email");
    }
  ...
  async view(criteria: object): Promise<IUserView> {
    const user = await User.findOne({ ...criteria, deleted: false }).select({
      password: 0,
    });
    ...
    if (user.role != USER_TYPES.admin) {
      user_type = await userModel[user.role]
  ...
  async downloadUserData(userType: string) {
    const users = await userModel[userType]
      .find({})
      .populate({ path: "user", select: "fullname email phone createdAt" });
    ...
    const columns = [
      ...
      {
        label: "Signed up on",
        value: (row: any) => row.user.createdAt,
        format: "d-mmm-yy",
      },
    ];
